...
class MinimaxEngine:
    ...
    def bestAction(self, state, selection, is_max):
        start_time = time.time()
        possibleMoves = getPossibleMoves(selection,state) # List of all states     #### This function is needed ####
        baseScore = -9999
        finalMove = None
        ...
        for moves in possibleMoves:
            ...
            bestMove = max(baseScore, self.minimax(self.max_depth, moves, -10000, 10000, not is_max, selection))
    ...
    def minimax(self, depth, state, alpha, beta, is_max, selection):
        ...
        if(is_max):
            ...
            for state in possibleMoves:
                ...
                bestMove = max(bestMove, self.minimax(depth-1, state, alpha, beta, not is_max, selection))
            ...
            for state in possibleMoves:
                # print("state2: ")
                # print(state)
                bestMove = min(bestMove, self.minimax(depth-1, state, alpha, beta, not is_max, selection))
    ...
    def move(self,x1,y1,x2,y2,piece_name,selection):
        ...
        '''if piece_name == "+1":
            ...
            try:
                ...
                pass'''
        return new_state
def getPossibleMoves(selection,state):
    total_states = []
    piece_states = []
    piece_actions = []
    if is_unavailable("-1",selection) == False:
    ...
    if is_unavailable("-2",selection) == False:
    ...
    if is_unavailable("-3",selection) == False:
    ...
    if is_unavailable("-4",selection) == False:
    ...
    if is_unavailable("-5",selection) == False:
    ...
    if is_unavailable("-6",selection) == False:
...
def is_unavailable(piece,selection):
...
if __name__ == "__main__":
    ...
    while True:
        if iterator%2 == 0:
            ...
            pawns_list = is_unavailable("+1",selection)
            knights_list = is_unavailable("+2",selection)
            bishops_list = is_unavailable("+3",selection)
            rooks_list = is_unavailable("+4",selection)
            queens_list = is_unavailable("+5",selection)
            kings_list = is_unavailable("+6",selection)
            ...
            if pawns_list and knights_list and bishops_list and rooks_list and queens_list and kings_list:
                ...
                break
            ai = input("Choose an ai:('1'= Tree-based AI, '2' = Baseline AI, 'q' = quit)")
            if ai == 'q': break
            elif ai == '1':
                print("Tree-based AI is playing...")
                max_depth = 1
                AI = MinimaxEngine(max_depth)
                new_state = AI.bestAction(state,selection,True)
